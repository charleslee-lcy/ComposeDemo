package com.charleslee.composedemo.flip;
...
public class FlipView extends FrameLayout {
	public interface OnFlipListener {
		/**
		 * onPageSelected 翻折完成回调
		 */
		void onFlippedToPage(FlipView v, int position, long id);

		/**
		 * onPageScrolled 翻折中回调
		 */
		void onFlipping(FlipView v);
	}
	...
	private void init() {
		...
		mShinePaint.setColor(Color.parseColor("#22000000"));
		mShinePaint.setStyle(Style.FILL);

//		setCameraDistance(Integer.MAX_VALUE);
	}
	...
	private void setFlipDistance(float flipDistance) {
		Log.i("charleslee", "flipDistance: -> " + flipDistance);
		if (mPageCount < 1) {
			mFlipDistance = 0;
			mCurrentPageIndex = INVALID_PAGE_POSITION;
			mCurrentPageId = -1;
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			startFlipDistance = mFlipDistance;
...
//				float deltaUserTouch = (mFlipDistance - mCurrentPageIndex * FLIP_DISTANCE_PER_PAGE) > 90 ? getHeight() : getHeight() * 1.0f / 3;

				deltaFlipDistance /= ((isFlippingVertically() ? getHeight() * 1.0f / 3
						: getWidth()) / FLIP_DISTANCE_PER_PAGE);


				Log.i("glq", "glq: " + (mFlipDistance - startFlipDistance));
				Log.i("deltaFlipDistance", "deltaFlipDistance: " + deltaFlipDistance);

				float tempDistance = mFlipDistance - startFlipDistance + deltaFlipDistance;
				...
				if (mode == 1) {
					// 上翻
					if (tempDistance > 180) {
						mFlipDistance = startFlipDistance + 180;
						return true;
					}
					if (tempDistance < 0) {
						mFlipDistance = startFlipDistance;
						return true;
					}
				} else {
					// 下翻
				...
				if (isOverFlipping) {
					...
					if (mOnOverFlipListener != null) {
						...
						mOnOverFlipListener.onOverFlip(this, mOverFlipMode,
								mCurrentPageIndex == 0, 0, FLIP_DISTANCE_PER_PAGE);
					}
				}
			}
			break;
	...
	private void positionMatrix() {
		mMatrix.preScale(0.05f, 0.05f);
		mMatrix.postScale(20f, 20f);
...
//				showItemTabLayout(mPreviousPage.v, false);
//				showItemTabLayout(mCurrentPage.v, false);
//				showItemTabLayout(mNextPage.v, false);
			}
		});
	}

	private void postFlipping() {
		post(() -> {
			if (mOnFlipListener != null) {
				mOnFlipListener.onFlipping(FlipView.this);
//				showItemTabLayout(mPreviousPage.v, true);
//				showItemTabLayout(mCurrentPage.v, true);
//				showItemTabLayout(mNextPage.v, true);
			}
		});
	}

	private void showItemTabLayout(View view, boolean show) {
		if (view != null) {
			view.findViewById(R.id.tab_layout).setVisibility(show ? View.VISIBLE : View.GONE);
		}
	}
	...
	private void peak(boolean next, boolean once) {
		...
		if (next) {
			mPeakAnim = ValueAnimator.ofFloat(baseFlipDistance,
					baseFlipDistance + FLIP_DISTANCE_PER_PAGE);
		} else {
			mPeakAnim = ValueAnimator.ofFloat(baseFlipDistance,
					baseFlipDistance - FLIP_DISTANCE_PER_PAGE);
		}
	...
	/* ---------- API ---------- */
	public void setAdapterWithTabLayout(ListAdapter adapter, TabLayout tabLayout) {
	...
	public void flipTo(int page) {
		if (page < 0) page = 0;
		if (page > mPageCount - 1) page = mPageCount - 1;
	...
	public void backToTop() {
		final int start = mCurrentPageIndex < 6 ? (int) mFlipDistance : 5 * FLIP_DISTANCE_PER_PAGE;
		final int delta = -start;
	...
	public void backToBottom() {
		final int start = mPageCount - mCurrentPageIndex < 6 ? (int) mFlipDistance : (mPageCount - 5) * FLIP_DISTANCE_PER_PAGE;
		final int delta = (mPageCount - 1) * FLIP_DISTANCE_PER_PAGE - start;
	...
	public void peakNext() {
		smoothFlipTo(mCurrentPageIndex + 1);
//		if (mCurrentPageIndex < mPageCount - 1) {
//			peak(true, once);
//		}
	}
